    ...
    function renderCytoscape(data, maxPagerank, maxWeight, siloLabels) {
        ...
        cy.on('tap', 'edge', function(evt) {
            ...
            $.post(silGraphData.ajaxurl || ajaxurl, {
                ...
                if (!response.success) {
                    $content.html('<p style="color:#d63638; padding:20px;">Erreur : ' + (response.data || 'Impossible de charger le contexte.') + '</p>');
                    return;
                }
                const d = response.data;
                const sourceColor = getColorForCluster(d.source_silo);
                const targetColor = getColorForCluster(d.target_silo);
                ...
                html += '<div style="flex:1; text-align:right;"><small style="display:block; color:#64748b; font-size:9px;">DE</small><strong style="font-size:11px;">' + d.source_title + '</strong></div>';
                html += '<div style="color:#94a3b8;">➔</div>';
                html += '<div style="flex:1;"><small style="display:block; color:#64748b; font-size:9px;">VERS</small><strong style="font-size:11px;">' + d.target_title + '</strong></div>';
                html += '</div>';
                ...
                if (d.is_leak) {
                    ...
                    html += '<div><strong style="color:#b91c1c;">Fuite Sémantique !</strong><br><span style="color:#7f1d1d; font-size:11px;">Ce lien traverse deux silos différents : <strong>' + d.source_silo_label + '</strong> vers <strong>' + d.target_silo_label + '</strong>.</span></div>';
                    html += '</div>';
                } else {
                    ...
                    html += '<div><strong style="color:#166534;">Maillage Hermétique</strong><br><span style="color:#14532d; font-size:11px;">Ce lien reste à l\'intérieur du silo : <strong>' + d.source_silo_label + '</strong>.</span></div>';
                    html += '</div>';
                }
                ...
                html += '"...' + (d.context_prev || '') + ' <span style="background:#fde047; padding:2px 4px; border-radius:3px; font-weight:700; font-style:normal;">' + d.anchor + '</span> ' + (d.context_next || '') + '..."';
                html += '</div>';
                html += '</div>';
                ...
                html += '<button class="button ' + nfClass + ' sil-toggle-nofollow-btn" data-source="' + sourceId + '" data-target-url="' + d.target_url + '" style="font-size:12px;">' + nfText + '</button>';
                
                // Delete Link
                html += '<button class="button sil-delete-link-edge-btn" data-source="' + sourceId + '" data-target-url="' + d.target_url + '" style="font-size:12px; color:#dc2626; border-color:#fca5a5;">🗑️ Supprimer le lien</button>';
                
                html += '</div>';

                // Navigation
                html += '<div style="margin-top:20px; border-top:1px solid #e2e8f0; padding-top:15px; display:flex; gap:8px;">';
                if (d.source_edit_url) html += '<a href="' + d.source_edit_url + '" target="_blank" class="button button-secondary" style="flex:1; text-align:center; text-decoration:none; font-size:11px;">Editer Source</a>';
                if (d.target_edit_url) html += '<a href="' + d.target_edit_url + '" target="_blank" class="button button-secondary" style="flex:1; text-align:center; text-decoration:none; font-size:11px;">Editer Cible</a>';
                html += '</div>';
