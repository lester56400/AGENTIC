    ...
    function renderCytoscape(data, maxPagerank, maxWeight, siloLabels) {
        ...
        cy.on('tap', 'edge', function(evt) {
            ...
            $.post(silGraphData.ajaxurl || ajaxurl, {
                ...
                // Semantic Leak Alert
                if (d.is_leak) {
                    html += '<div style="background:#fef2f2; border:1px solid #fca5a5; border-radius:6px; padding:10px; margin-bottom:15px; font-size:12px; display:flex; align-items:flex-start; gap:8px;">';
                    html += '<span style="font-size:18px;">⚠️</span>';
                    html += '<div><strong style="color:#b91c1c;">Fuite Sémantique !</strong><br><span style="color:#7f1d1d; font-size:11px;">Ce lien traverse deux silos différents : <strong>' + escapeHtml(d.source_silo_label) + '</strong> vers <strong>' + escapeHtml(d.target_silo_label) + '</strong>.</span></div>';
                    html += '</div>';
                } else {
