...
class SIL_Pilot_Engine {
    ...
    public function self_diagnose() {
        ...
        $diagnosis = [
            ...
            'semantic' => [
                'total_embeddings' => (int) $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}postmeta WHERE meta_key = '_sil_embedding' AND meta_value != ''"),
                'pca_ready' => 'Vérification...',
                'cache_version' => 'v11.0'
            ],
            'counts' => [
            ...
            'silo_cluster_health' => (function() {
                try {
                    return $this->diagnose_silo_clusters();
                } catch (\Throwable $e) {
        ...
        // Diagnostic PCA
        $diagnosis['semantic']['pca_ready'] = ($diagnosis['semantic']['total_embeddings'] >= 3) ? '✅ PRÊT (' . $diagnosis['semantic']['total_embeddings'] . ' vecteurs)' : '❌ INSUFFISANT (Besoin de 3 minimum)';
    ...
    /**
     * Diagnostic spécifique pour le moteur de silotage (V2.8.4)
     * Identifie les causes de collapse des silos.
     */
    private function diagnose_silo_clusters() {
        global $wpdb;
        ...
        // 1. Distribution SQL réelle
        $dist = $wpdb->get_results("SELECT silo_id, COUNT(*) as count FROM $table_membership WHERE is_primary = 1 GROUP BY silo_id");
        $diag['distribution'] = [];
        foreach ($dist as $r) {
            $diag['distribution']["Silo " . $r->silo_id] = (int)$r->count . " articles";
        }

        // 2. État des Centroïdes (Mémoire vs DB)
        $centroids = $this->main->semantic_silos->get_silo_centroids();
        $diag['centroids'] = [
            'count' => is_array($centroids) ? count($centroids) : 0,
            'status' => (!empty($centroids) && is_array($centroids)) ? '✅ Présents' : '❌ ABSENTS (Reconstruction requise)',
            'zero_vectors' => 0,
            'collisions' => 0
        ];
        
        $c_ids = is_array($centroids) ? array_keys($centroids) : [];
        for ($i=0; $i < count($c_ids); $i++) {
            $c1 = $centroids[$c_ids[$i]];
            if (array_sum($c1) == 0) $diag['centroids']['zero_vectors']++;
            
            for ($j=$i+1; $j < count($c_ids); $j++) {
                $c2 = $centroids[$c_ids[$j]];
                $sim = $this->main->cosine_similarity($c1, $c2);
                if ($sim > 0.999) $diag['centroids']['collisions']++;
            }
        }

        // 3. Paramètres Mathématiques (Stretching)
        $rebuild_stats = get_option('sil_last_iteration_stats');
        $diag['math_params'] = [
            'niche_diameter' => get_option('sil_niche_diameter', 'Non défini'),
            'fuzziness_m' => isset($rebuild_stats['m']) ? $rebuild_stats['m'] : 1.2,
            'mode' => get_option('sil_silo_mode', 'auto'),
            'manual_target' => get_option('sil_semantic_k', 6)
        ];

        // 4. Couverture des Embeddings
        $published_count = (int) $wpdb->get_var("SELECT COUNT(*) FROM $wpdb->posts WHERE post_type IN ('post', 'page') AND post_status = 'publish'");
        $emb_count = (int) $wpdb->get_var("SELECT COUNT(*) FROM $table_embeddings");
        $diag['coverage'] = [
            'published_posts' => $published_count,
            'with_embeddings' => $emb_count,
            'missing' => max(0, $published_count - $emb_count),
            'health' => ($published_count === $emb_count) ? '✅ 100%' : '⚠️ Partiel'
        ];

        // 5. Cohérence du Cache & Itérations
        $graph_cache = get_transient('sil_graph_cache_v13_0');
        $rebuild_state = get_transient('sil_rebuild_state'); // Pendant le calcul
        $rebuild_stats = get_option('sil_last_iteration_stats'); // Après le calcul
        
        $last_iter = $rebuild_state ? $rebuild_state['iter'] : ($rebuild_stats['iter'] ?? 'N/A');
        $last_delta = $rebuild_state ? ($rebuild_state['last_delta'] ?? 'N/A') : ($rebuild_stats['delta'] ?? 'N/A');
        
        $diag['cache'] = [
            'audit_cache' => $graph_cache ? '✅ Actif' : '🟡 Vide',
            'last_rebuild' => get_option('sil_last_semantic_rebuild', 'Jamais'),
            'last_iter' => $last_iter,
            'last_delta' => is_numeric($last_delta) ? round($last_delta, 6) : $last_delta
        ];
