...
class SIL_Admin_Manager {
    ...
    public function register_settings() {
        ...
        $settings = [
            ...
            'sil_semantic_k'          => 'absint',
            'sil_silo_mode'           => 'sanitize_text_field',
            'sil_silo_ambition'       => 'sanitize_text_field',
            'sil_node_repulsion'      => 'absint',
            'sil_component_spacing'   => 'absint',
            'sil_gravity'             => 'floatval',
            'sil_gsc_overlap_threshold' => 'absint'
        ];
    ...
    public function enqueue_admin_scripts($hook) {
        ...
        if (strpos($hook, 'sil-cartographie') !== false) {
            ...
            wp_enqueue_script('sil-audit-engine', SIL_PLUGIN_URL . 'assets/sil-audit-engine.js', ['jquery'], time(), true);
            wp_enqueue_script('sil-graph-js', SIL_PLUGIN_URL . 'assets/sil-graph-v3.js', ['jquery', 'cytoscape', 'sil-bridge-manager', 'sil-audit-engine'], time(), true);
            wp_enqueue_style('sil-graph-css', SIL_PLUGIN_URL . 'assets/sil-graph-v2.css', [], time());
        }

        // Pilotage Specifics
        if (strpos($hook, 'sil-pilotage') !== false) {
            wp_enqueue_style('sil-pilot-center', SIL_PLUGIN_URL . 'assets/sil-pilot-center.css', [], time());
            wp_enqueue_script('sil-pilot-center-js', SIL_PLUGIN_URL . 'assets/sil-pilot-center.js', ['jquery', 'sil-bridge-manager'], time(), true);
        }
    ...
    public function save_cornerstone_meta_box($post_id) {
        ...
        if (current_user_can('edit_post', $post_id)) {
            ...
            // Semantic Exclusion
            if (isset($_POST['sil_exclude_from_mapping'])) {
                update_post_meta($post_id, '_sil_exclude_from_mapping', '1');
                delete_transient('sil_graph_cache'); // Force refresh graph
            } else {
                delete_post_meta($post_id, '_sil_exclude_from_mapping');
            }
        }
    }
