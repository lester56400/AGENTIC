...
class SIL_Cluster_Analysis {
    ...
    public function get_graph_data() {
        ...
        foreach ( array_keys($unique_clusters) as $cid ) {
            $label = "Silo $cid";
            if (isset($cluster_cats[$cid])) {
                $counts = array_count_values($cluster_cats[$cid]); arsort($counts);
                $label = "Thématique : " . array_key_first($counts);
            }
            $nodes_data[] = ['data' => ['id' => 'silo_'.$cid, 'label' => $label, 'is_silo_parent' => "true", 'cluster_id' => $cid]];
        }
