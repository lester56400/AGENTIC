...
class Sil_Gsc_Sync
{
    ...
    public function sync_data($post_ids = [])
    {
        ...
        if (!$this->is_configured()) {
            ...
            return [
                'success' => false,
                'message' => 'GSC n\'est pas configuré.'
            ];
        }

        $total_keywords_in_batch = 0;
        ...
        try {
            ...
            if (empty($rows)) {
                return [
                    'success' => true,
                    'keywords_saved' => 0,
                    'message' => 'Aucune donnée GSC trouvée pour cette période.'
                ];
            }
            ...
            foreach ($pages_data as $url => $data) {
                ...
                if ($post_id) {
                    ...
                    // Increment keywords saved counter for THIS batch call
                    $total_keywords_in_batch += count($top_queries);
                    
                    // Also update the global transient for overall tracking
            ...
            // --- NOUVEAU: Détecter les cannibalisation (Duel de Pages) ---
            if (class_exists('SIL_Pilot_Engine')) {
                // On s'assure d'avoir l'instance correcte
                $main = class_exists('SmartInternalLinks') ? \SmartInternalLinks::get_instance() : $GLOBALS['smart_internal_links'];
                $pilot = new SIL_Pilot_Engine($main);
                $pilot->detect_cannibalization_risks();
            }

            update_option('sil_gsc_last_sync', current_time('mysql'), false);
            return [
                'success' => true,
                'keywords_saved' => $total_keywords_in_batch,
