...
class SIL_Centrality_Engine
{
    ...
    public static function get_representativeness_score($post_embedding, $barycenter)
    {
        ...
        if ($main && isset($main->embeddings)) {
            return $main->embeddings->calculate_similarity($post_embedding, $barycenter);
        }
    ...
    /**
     * Récupère les embeddings pour un lot de textes via l'API OpenAI.
     * @param array $texts Tableau de chaînes de caractères.
     * @param string $api_key Clé API OpenAI.
     * @param string $model Modèle à utiliser (défaut: text-embedding-3-small).
     * @return array Tableau d'embeddings indexé par l'ordre d'entrée, ou vide en cas d'erreur.
     */
    public static function batch_get_embeddings($texts, $api_key, $model = 'text-embedding-3-small')
    {
        $main = SmartInternalLinks::get_instance();
        if ($main && isset($main->embeddings)) {
            return $main->embeddings->batch_get_embeddings($texts);
        }
