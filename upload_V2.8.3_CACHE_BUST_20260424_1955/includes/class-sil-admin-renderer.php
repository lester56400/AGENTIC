...
<span
				...
				<div class="sil-card-body" style="padding:15px;">
					<form method="post" action="options.php">
						...
						<table class="form-table">
							...
							<tr>
								<th><label for="sil_semantic_k">Nombre de Cocons Sémantique (k)</label></th>
								<td>
									...
									<p class="description">Le nombre de grappes (clusters) que l'IA va tenter d'isoler. 6 par défaut. 
                                        <strong>Recommandé : <?php echo $this->plugin->semantic_silos->calculate_recommended_k(); ?>-<?php echo $this->plugin->semantic_silos->calculate_recommended_k() + 2; ?></strong>
                                    </p>
								</td>
							</tr>
							<tr>
	...
	public function render_cartographie_page() {
		...
		<div class="sil-pilot-wrap sil-swiss-mode">
			<header class="sil-pilot-header">
				...
				<div class="sil-pilot-controls" style="flex:1; justify-content: flex-end; gap:10px;">
					<select id="sil-silo-filter" style="margin:0;">
					...
					<div style="display:flex; flex-direction:column; gap:5px;">
						<div style="display:flex; align-items:center; gap:5px;">
							<select id="sil-tension-selector" style="margin:0; border:2px solid #000; font-weight:900; background:#000; color:#fff; cursor:pointer;" title="Audit de pertinence sémantique">
								<option value="all">🌐 Vue globale (Tout voir)</option>
								<option value="audit_weak">✂️ Audit : Liens à couper (0-25%)</option>
								<option value="audit_fragile">⚠️ Audit : Liens fragiles (25-50%)</option>
								<option value="audit_robust">✅ Audit : Liens robustes (50-75%)</option>
								<option value="audit_core">💎 Audit : Cœur de Silo (> 75%)</option>
							</select>
							<button id="sil-apply-audit" class="button" style="background:#000; color:#fff; border:2px solid #000; font-weight:900; height:32px; padding:0 15px; cursor:pointer;">FILTRER 🔍</button>
						</div>
						<div id="sil-audit-progress-container" style="display:none; width:100%; height:4px; background:#1f1f23; border-radius:2px; overflow:hidden;">
							<div id="sil-audit-progress-bar" style="width:0%; height:100%; background:#fff; transition: width 0.2s ease;"></div>
						</div>
						<div id="sil-audit-status" style="font-size:10px; color:#94a3b8; font-family:monospace; display:none;"></div>
					</div>
					<input type="text" id="sil-node-search" list="sil-node-list" placeholder="🔍 Rechercher un article..." style="width: 250px; margin:0;">
					<datalist id="sil-node-list"></datalist>
				</div>
			</header>

			<div class="sil-graph-toolbar-swiss" style="background:#000; padding:10px 20px; display:flex; gap:10px; align-items:center; border:2px solid #000; margin-bottom:20px;">
				<button id="sil-refresh-graph" class="button" style="background:#fff; color:#000; border:2px solid #000; font-weight:900;">🔄 RAFRAÎCHIR</button>
				<button id="sil-center-graph" class="button" style="background:#fff; color:#000; border:2px solid #000;"><?php echo SIL_Icons::get_icon( 'target', [ 'size' => 16 ] ); ?> CENTRER</button>
				<button id="sil-zoom-in" class="button" style="background:#fff; color:#000; border:2px solid #000;">➕</button>
				<button id="sil-zoom-out" class="button" style="background:#fff; color:#000; border:2px solid #000;">➖</button>
				<button id="sil-export-png" class="button" style="background:#fff; color:#000; border:2px solid #000;">📸 PNG</button>
				<button id="sil-export-json" class="button" style="background:#fff; color:#000; border:2px solid #000;">📦 JSON</button>
				<span class="sil-badge-count" style="margin-left:auto; color:#fff; font-family:var(--sil-swiss-mono); font-weight:900;">NŒUDS: <span id="sil-node-count">0</span></span>
			</div>
	...
	public function render_cornerstone_box( $post ) {
		...
		<?php $is_pillar = get_post_meta( $post->ID, '_sil_is_pillar', true ); ?>
