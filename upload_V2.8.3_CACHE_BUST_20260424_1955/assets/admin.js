(function($) {
    ...
    window.silNotify = function (message, type) {
        ...
        var duration = (type === 'error') ? 15000 : 4000;
            ...
            function processNextChunk() {
                ...
                $.post(silSharedData.ajaxurl, {
                    ...
                    if (chunkResponse.success) {
                        ...
                        $btn.prop('disabled', false).text(originalText);
                        var rawErr = chunkResponse.data || 'Inconnue';
                        var cleanErr = (typeof rawErr === 'object') ? (rawErr.message || JSON.stringify(rawErr)) : rawErr;
                        var chunkErrMsg = 'Erreur sur le lot ' + (currentChunk + 1) + ': ' + cleanErr;
    ...
    // Rebuild Semantic Silos & Extract Entities (Stateful Pipeline Phase 2.7)
    $('#sil-generate-architecture').on('click', function () {
        var $btn = $(this);
        var originalText = $btn.text();
        var $status = $('#sil-silo-rebuild-status');

        if (!confirm('Cela va recalculer tous les cocons par étapes pour garantir la stabilité. Continuer ?')) return;

        $btn.prop('disabled', true).text('⏳ Initialisation...');
        $status.show().removeClass('sil-status-error sil-status-success').addClass('sil-status-pending').text('Préparation des données sémantiques...');

        // ÉTAPE 1 : Initialisation
        $.post(silSharedData.ajaxurl, {
            action: 'sil_rebuild_silos_step_init',
            nonce: silSharedData.nonce
        }, function (res) {
            if (res.success) {
                $status.text('Initialisation terminée. Début des itérations...');
                processSiloIterations();
            } else {
                handleSiloError(res.data);
            }
        }).fail(function() { handleSiloError('Erreur réseau lors de l\'initialisation'); });

        // ÉTAPE 2 : Boucle d'itérations
        function processSiloIterations() {
            $.post(silSharedData.ajaxurl, {
                action: 'sil_rebuild_silos_step_iterate',
                nonce: silSharedData.nonce
            }, function (res) {
                if (res.success) {
                    if (res.data.status === 'iterating') {
                        $status.text('Optimisation sémantique : Itération ' + res.data.iteration + ' / ' + res.data.max_iter + '...');
                        $btn.text('⏳ ' + Math.round((res.data.iteration / res.data.max_iter) * 100) + '%');
                        // Délai de confort pour le serveur
                        setTimeout(processSiloIterations, 300);
                    } else if (res.data.status === 'converged') {
                        $status.text('Convergence atteinte. Finalisation du maillage...');
                        finalizeSiloRebuild();
                    }
                } else {
                    handleSiloError(res.data);
                }
            }).fail(function() { handleSiloError('Le serveur est trop lent ou saturé. Réessayez.'); });
        }

        // ÉTAPE 3 : Finalisation
        function finalizeSiloRebuild() {
            $status.text('Persistance des résultats en base de données...');
            $.post(silSharedData.ajaxurl, {
                action: 'sil_rebuild_silos_step_finalize',
                nonce: silSharedData.nonce
            }, function (res) {
                if (res.success) {
                    var msg = silSharedData.icons.check + ' Silos générés. ' + (res.data.count || 0) + ' articles classés.';
                    $status.html(msg);
                    
                    // NOUVEAU : Enchaînement sur l'extraction des entités (Signatures)
                    $btn.text('🧠 Extraction des signatures...');
                    $status.text('Démarrage de l\'analyse des signatures détaillées...');
                    
                    function processExtractionBatch() {
                        $.post(silSharedData.ajaxurl, {
                            action: 'sil_extract_entities',
                            nonce: silSharedData.nonce
                        }, function(r) {
                            if (r.success) {
                                if (r.remaining > 0) {
                                    $status.text('Extraction en cours : ' + r.covered + ' / ' + r.total + ' articles...');
                                    setTimeout(processExtractionBatch, 400);
                                } else {
                                    $status.removeClass('sil-status-pending').addClass('sil-status-success').text('✅ Architecture et Signatures générées avec succès !');
                                    $btn.prop('disabled', false).text(originalText);
                                    silToast('Architecture générée avec succès', 'success');
                                }
                            } else {
                                handleSiloError(r.data);
                            }
                        }).fail(function() { handleSiloError('Erreur lors de l\'extraction des signatures.'); });
                    }
                    
                    setTimeout(processExtractionBatch, 800);
                } else {
                    handleSiloError(res.data);
                }
            }).fail(function() { handleSiloError('Erreur finale. Les données n\'ont peut-être pas été sauvées.'); });
        }

        function handleSiloError(msg) {
            $btn.prop('disabled', false).text('❌ Réessayer');
            $status.removeClass('sil-status-pending').addClass('sil-status-error').text('Erreur : ' + (msg || 'Inconnue'));
            silToast('Erreur dans le pipeline de silotage', 'error');
        }
    });
    ...
    $(document).on('click', '.sil-adopt-btn', function(e) {
        ...
        $.post(silSharedData.ajaxurl, {
            ...
            $btn.prop('disabled', false).text('Adopter');
            var msg = 'Erreur serveur (Adoption). ';
            if (xhr.status === 500) msg += 'Le serveur a mis trop de temps à répondre ou a rencontré une erreur fatale.';
            silToast(msg, 'error');
            console.error('SIL Adoption Error:', status, error, xhr.responseText);
        });
    });

    function openAdoptionModal(orphanId, data) {
        let parentsHtml = '';
        if (data.parents && data.parents.brother) {
            parentsHtml += `
                <label class="anchor-option" style="display:flex; align-items:center; margin-bottom:8px; padding:10px; border:1px solid #e2e8f0; border-radius:6px; cursor:pointer;">
                    <input type="radio" name="sil-selected-parent" value="${data.parents.brother.id}" checked>
                    <span class="anchor-text" style="margin-left:8px; flex:1;"><strong>👬 Frère Sémantique :</strong> ${escHtml(data.parents.brother.title)}</span>
                </label>`;
        }
        if (data.parents && data.parents.megaphone) {
            const isChecked = (!data.parents.brother) ? 'checked' : '';
            const warningBadge = data.parents.megaphone.already_linking ? '<span class="sil-badge-warning" style="margin-left:8px;font-size:10px;padding:2px 6px;" title="Le Mégaphone fait déjà un lien vers cet article">⚠️ Déjà lié</span>' : '';
            parentsHtml += `
                <label class="anchor-option" style="display:flex; align-items:center; padding:10px; border:1px solid #e2e8f0; border-radius:6px; cursor:pointer;">
                    <input type="radio" name="sil-selected-parent" value="${data.parents.megaphone.id}" ${isChecked}>
                    <span class="anchor-text" style="margin-left:8px; flex:1;"><strong>🏗️ Mégaphone :</strong> ${escHtml(data.parents.megaphone.title)}</span>
                    ${warningBadge}
                </label>`;
        }
                ...
                <div class="sil-modal-content">
                    ...
                    <div class="sil-modal-body">
                        <p class="sil-modal-desc">
                            Choisissez la source qui fera le lien vers cet article. L'IA recommande le Frère Sémantique pour diversifier le maillage.
                        </p>
                        
                        <div class="sil-anchor-selection" style="margin-bottom: 20px;">
                            <span class="section-title">1. Source du lien (Parent) :</span>
                            <div class="parent-options" style="margin-top:10px;">
                                ${parentsHtml}
                            </div>
                        </div>

                        <div class="sil-anchor-selection">
                            <span class="section-title">2. Choisissez une ancre :</span>
                    ...
                    <div class="sil-modal-footer">
                        ...
                        <button id="sil-confirm-adopt" class="sil-btn sil-btn-secondary" data-orphan-id="${orphanId}">Insertion classique</button>
                        <button id="sil-bridge-adopt" class="sil-btn sil-btn-primary" data-orphan-id="${orphanId}" title="Créer une transition fluide via IA">🌉 Pont Sémantique (Recommandé)</button>
                    </div>
                </div>
            </div>
        `;
    ...
    $(document).on('click', '#sil-bridge-adopt', function() {
        ...
        const parentId = $('input[name="sil-selected-parent"]:checked').val();
        ...
        if (window.SIL_Bridge) {
            window.SIL_Bridge.generate(parentId, orphanId, anchor, $btn);
        } else {
    ...
    $(document).on('click', '#sil-run-semantic-audit', function(e) {
        ...
        $.post(silSharedData.ajaxurl, { action: 'sil_run_semantic_audit', nonce: silSharedData.nonce }, function(r) {
            ...
            if(r && r.success) {
                ...
                const total = r.data.total || nodes;
                const msg = `<div class="notice notice-success inline" style="margin:0;"><p>✅ <strong>Audit terminé sur ${nodes} / ${total} contenus !</strong> Les algorithmes ont recalculé la cohérence.<br/><br/>👉 <strong>Allez maintenant dans l'onglet "Cartographie"</strong> pour voir les résultats du maillage.</p></div>`;
        ...
        let html = `
            <div style="margin-top:20px;">
                ...
                <div style="margin-bottom:12px; display:flex; gap:8px;">
                    <button id="sil-repatriate-all" class="button button-primary" style="background:#10b981; border-color:#059669;">🔄 Repatrier tous les intrus (${intruders.length})</button>
                </div>
        ...
        intruders.forEach(item => {
            ...
            html += `
                <tr data-post-id="${item.id}">
                    <td><strong>${item.title}</strong><br><small style="color:#64748b;">ID: ${item.id}</small></td>
                    <td><span class="badge" style="background:#fecaca; color:#991b1b; padding:3px 8px; border-radius:12px;">#${item.current}</span></td>
                    <td><span class="badge" style="background:#d1fae5; color:#065f46; padding:3px 8px; border-radius:12px; font-weight:bold;">→ #${item.ideal}</span></td>
                    <td><span style="color:#059669; font-weight:bold;">${roiIndex.toLocaleString()} pts</span></td>
                    <td>
                        <button class="button button-small sil-repatriate-btn" data-post-id="${item.id}" style="background:#10b981; color:#fff; border-color:#059669;">🔄 Repatrier</button>
                        <button class="button button-small sil-auto-bridge-btn" data-target-cluster="${item.ideal}">🌉 Pont</button>
                    </td>
                </tr>`;
        });
    ...
    // --- REPATRIATION DES INTRUS (Phase 0.1 — Option A : Ralliement Sémantique) ---
    $(document).on('click', '.sil-repatriate-btn', function() {
        const $btn = $(this);
        const postId = $btn.data('post-id');
        
        $btn.prop('disabled', true).text('⏳ Repatriation...');
        ...
        $.post(silSharedData.ajaxurl, {
            ...
            if (r.success) {
                $btn.text('✅ Fait').closest('tr').css('opacity', '0.5');
                silToast(r.data.message, 'success');
            } else {
                $btn.prop('disabled', false).text('🔄 Repatrier');
                const errorMsg = (r && r.data) ? (typeof r.data === 'object' ? (r.data.message || JSON.stringify(r.data)) : r.data) : 'Inconnue';
    ...
    // Repatrier tous les intrus en séquence
    $(document).on('click', '#sil-repatriate-all', function() {
        const $btn = $(this);
        const $rows = $container => $container.find('.sil-repatriate-btn:not(:disabled)');
        const $allBtns = $('.sil-repatriate-btn:not(:disabled)');
        const total = $allBtns.length;
        
        if (total === 0) return;
        if (!confirm(`Repatrier ${total} intrus vers leur silo sémantique idéal ?`)) return;

        $btn.prop('disabled', true).text(`⏳ 0/${total}...`);
        let processed = 0;

        function processNext() {
            const $nextBtn = $('.sil-repatriate-btn:not(:disabled)').first();
            if ($nextBtn.length === 0) {
                $btn.text(`✅ ${processed}/${total} repatriés`);
                silToast(`${processed} intrus repatriés avec succès.`, 'success');
                return;
            }
            $nextBtn.trigger('click');
            processed++;
            $btn.text(`⏳ ${processed}/${total}...`);
    ...
    function runEmbeddingBatch() {
        ...
        $.post(silSharedData.ajaxurl, { action: 'sil_index_embeddings_batch', nonce: silSharedData.nonce }, function(r) {
            if (r && r.success) {
                ...
                const errorMsg = (r && r.data) ? (typeof r.data === 'object' ? (r.data.message || JSON.stringify(r.data)) : r.data) : 'Erreur API ou Timeout';
                silToast('Erreur: ' + errorMsg, 'error');
