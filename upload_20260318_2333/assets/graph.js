jQuery(document).ready(function ($) {
    ...
    function renderCytoscape(data, maxImp, maxWeight, siloLabels) {
            ...
            style: [
                {
                    ...
                    style: {
                        'label': 'data(label)',
                        // 1. TAILLES AUGMENTÉES (min 60, max 140)
                        'width': `mapData(gsc_impressions, 0, ${maxImp > 1 ? maxImp : 10}, 60, 140)`,
                        'height': `mapData(gsc_impressions, 0, ${maxImp > 1 ? maxImp : 10}, 60, 140)`,
                        'background-color': n => getColorForCluster(n.data('cluster_id')),
                        'color': '#0f172a',
                        // 2. TEXTE ET ICÔNES PLUS GRANDS
                        'font-size': '15px',
                        'font-weight': 'bold',
                        // 3. MAGIE : Reste lisible même en dézoomant la carte
                        'min-zoomed-font-size': 10,
                        'text-valign': 'bottom',
                        'text-margin-y': '6px',
                        // 4. SURLIGNAGE BLANC SOUS LE TEXTE (Lisibilité parfaite en cas de chevauchement)
        ...
        const layout = cy.layout({
            ...
            idealEdgeLength: 100,              // Longueur normale pour les liens existants
            nodeOverlap: 20,
            refresh: 20,
            fit: true,
            padding: 50,
            randomize: false,
            // --- VARIABLES DYNAMIQUES DEPUIS LES RÉGLAGES ---
            componentSpacing: parseInt(silGraphData.spacing) || 60,              
            nodeRepulsion: function( node ){ 
                let baseRepulsion = parseInt(silGraphData.repulsion) || 300000;
                return baseRepulsion + (node.width() * 100); 
            }, 
            gravity: parseFloat(silGraphData.gravity) || 2.0,                      
            // ------------------------------------------------
        ...
        $(document).on('click', '.sil-delete-link-btn', function() {
            const $btn = $(this);
    ...
    window.openBridgeModal = function(data) {
        ...
        const modalHtml = `
        <div id="sil-ai-modal-overlay" style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(15,23,42,0.8);z-index:999999;display:flex;justify-content:center;align-items:center;backdrop-filter:blur(4px);">
            <div style="background:#fff;width:95%;max-width:1000px;border-radius:12px;box-shadow:0 25px 50px -12px rgba(0,0,0,0.25);display:flex;flex-direction:column;overflow:hidden;max-height:90vh;">
                <div style="padding:20px;background:#f8fafc;border-bottom:1px solid #e2e8f0;display:flex;justify-content:space-between;align-items:center;">
                    <h3 style="margin:0;font-size:18px;color:#0f172a;">🤖 Workflow IA : Création de Pont Sémantique</h3>
                    <button id="sil-ai-modal-close" style="background:#f1f5f9;border:1px solid #e2e8f0;border-radius:6px;width:32px;height:32px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:#64748b;">&times;</button>
                </div>
                <div style="display:flex;flex-direction:row;flex-wrap:wrap;overflow-y:auto;background:#fff;">
                    <div style="flex:1;min-width:320px;padding:24px;border-right:1px solid #f1f5f9;background:#f8fafc;box-sizing:border-box;">
                        <label style="font-weight:700;display:block;margin-bottom:8px;color:#1e293b;font-size:13px;">1. Copiez ce Prompt :</label>
                        <p style="font-size:11px;color:#64748b;margin-bottom:15px;">Utilisez-le dans Gemini/ChatGPT pour générer le lien contextuel.</p>
                        <textarea id="sil-prompt-text" readonly style="width:100%;height:280px;padding:12px;font-family:'SFMono-Regular',Consolas,monospace;font-size:12px;border:1px solid #cbd5e1;background:#fff;resize:none;border-radius:6px;box-sizing:border-box;color:#334155;line-height:1.5;">${promptText}</textarea>
                        <button class="button" style="margin-top:10px;width:100%;" onclick="document.getElementById('sil-prompt-text').select();document.execCommand('copy');this.innerText='✅ Copié !';">Copier le prompt</button>
                    </div>
                    <div style="flex:1;min-width:320px;padding:24px;box-sizing:border-box;">
                        <label style="font-weight:700;display:block;margin-bottom:8px;color:#1e293b;font-size:13px;">2. Collez le résultat HTML ici :</label>
                        <p style="font-size:11px;color:#64748b;margin-bottom:15px;">Assurez-vous que le texte contient bien la balise <code>&lt;a href="..."&gt;</code>.</p>
                        <textarea id="sil-ai-modal-editor" placeholder="Collez ici le paragraphe généré par l'IA..." style="width:100%;height:280px;padding:12px;font-family:'SFMono-Regular',Consolas,monospace;font-size:12px;border:2px solid #3b82f6;border-radius:8px;box-sizing:border-box;background:#f0f9ff;color:#0c4a6e;line-height:1.5;"></textarea>
                    </div>
                </div>
                <div style="padding:15px 24px;background:#f8fafc;border-top:1px solid #e2e8f0;text-align:right;">
                    <button id="sil-ai-modal-cancel" class="button" style="margin-right:10px;padding:6px 16px;">Annuler</button>
                    <button id="sil-ai-modal-confirm" class="button button-primary" style="padding:6px 20px;font-weight:600;" data-source="${data.source_id}" data-target="${data.target_id}" data-original="${data.original}">Sauvegarder l'insertion</button>
                </div>
            </div>
        </div>`;
