...
class SIL_Cluster_Analysis {
    ...
    public function get_graph_data($force_refresh = false) {
        ...
        foreach ( $posts as $post ) {
            ...
            $nodes_data[] = [
                'data' => [
                    ...
                    'label' => get_the_title($post->ID), 'title' => get_the_title($post->ID), 'in_degree' => $in, 'out_degree' => $out,
        ...
        foreach($nodes_data as &$n) {
            $n['data']['sil_pagerank'] = round(($n['raw_pr'] / $max_raw_pr) * 100);
            unset($n['raw_pr']);
        }
            ...
            foreach ($unique_clusters as $cid => $exists) {
                ...
                if ($max_sim < 0.60) {
                    ...
                    $semantic_gaps[] = [
                        ...
                        'lexical_field' => array_unique(array_filter($lexical)),
                        'linking_strategy' => [
                            'receives_from' => array_map(function($pid) use ($post_titles) {
                                return ['id' => $pid, 'title' => $post_titles[$pid] ?? "Post $pid", 'suggested_anchor' => 'Maillage Pilier'];
                    ...
                    if ($dist < 0.40) {
                        ...
                        if (!$has_bridge) {
                            ...
                            $semantic_gaps[] = [
                                ...
                                'lexical_bridge' => array_unique(array_filter($bridge_lexical)),
                                'linking_strategy' => [
                                    'connects' => [$s1, $s2]
                                ]
                            ];
        ...
        foreach ($edges_data as $e) {
            $src = (string)$e['data']['source'];
            $tgt = (string)$e['data']['target'];
            if (isset($valid_node_ids[$src]) && isset($valid_node_ids[$tgt])) {
                $final_edges[] = $e;
            }
        }
