jQuery(document).ready(function ($) {
            ...
            if (r && r.success) {
                ...
                r.data.forEach(t => {
                    ...
                    html += `<div style="padding:10px; background:#fff; border-left:4px solid ${color}; font-size:12px; box-shadow:0 1px 2px rgba(0,0,0,0.05); margin-bottom:5px; display:flex; align-items:center; gap:8px;">
                        ${t.status ? silAjax.icons.check : silAjax.icons.x} <strong>${t.name}</strong><br><span style="color:#64748b;">Détail : ${t.val}</span>
                    </div>`;
                });
                $res.html(html);
            }
        ...
        $.post(silAjax.ajaxurl, {
            ...
            function processNextChunk() {
                if (currentChunk >= chunks.length) {
                    // All finished
                    var successMsg = silAjax.icons.check + ' ' + totalKeywordsSaved + ' mots-clés enregistrés pour ' + totalPages + ' pages !';
                    $btn.html(silAjax.icons.check + ' Terminé');
    ...
    $('#sil-rebuild-semantic-silos').on('click', function () {
        ...
        $.post(silAjax.ajaxurl, {
            ...
            $btn.prop('disabled', false).text(originalText);
            if (res.success) {
                var msg = silAjax.icons.check + ' Silos recalculés ! ' + (res.data.count || 0) + ' articles classés, ' + (res.data.bridges || 0) + ' ponts détectés.';
    ...
    $(document).on('click', '.sil-preview-btn', function () {
        ...
        $.post(silAjax.ajaxurl, {
            ...
            if (r.success && r.links && r.links.length > 0) {
                var html = '<div class="sil-preview-box"><div class="sil-preview-title">' + silAjax.icons.link + ' ' + r.links.length + ' lien(s) suggéré(s)</div>';
                r.links.forEach(function (link, idx) {
                    var id = 'link-' + postId + '-' + idx;

                    // Gestion affichage Cornerstone
                    var cornerstoneBadge = link.is_cornerstone ? ' <span style="background:#fef3c7; color:#d97706; padding:2px 6px; border-radius:4px; font-size:10px; border:1px solid #fcd34d; font-weight:bold; display:inline-flex; align-items:center; gap:4px;">' + silAjax.icons.star + ' PILIER</span>' : '';
