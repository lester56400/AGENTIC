jQuery(document).ready(function ($) {
    ...
    $(document).on('click', '#sil-run-unit-tests', function(e) {
        ...
        $.post(silAjax.ajaxurl, { action: 'sil_run_deep_unit_tests', nonce: silAjax.nonce }, function(r) {
            $btn.prop('disabled', false).text('🚀 Relancer les tests');
            if (r && r.success) {
                let html = '';
                r.data.forEach(t => {
                    const color = t.status ? '#059669' : '#dc2626';
                    html += `<div style="padding:10px; background:#fff; border-left:4px solid ${color}; font-size:12px; box-shadow:0 1px 2px rgba(0,0,0,0.05); margin-bottom:5px;">
                        ${t.status ? '✅' : '❌'} <strong>${t.name}</strong><br><span style="color:#64748b;">Détail : ${t.val}</span>
                    </div>`;
                });
                $res.html(html);
            }
    ...
    $('#sil-force-gsc-sync-main, #sil-force-gsc-sync').on('click', function () {
        ...
        $.post(silAjax.ajaxurl, {
            action: 'sil_get_all_ids_for_gsc_sync',
            _ajax_nonce: silAjax.nonce
        }, function (resIDs) {
            ...
            function processNextChunk() {
                if (currentChunk >= chunks.length) {
                    // All finished
                    var successMsg = '✅ ' + totalKeywordsSaved + ' mots-clés enregistrés pour ' + totalPages + ' pages !';
                    $btn.text('✅ Terminé');
                    if ($status.length) $status.text(successMsg).css('color', 'green');
    ...
    $('#sil-rebuild-semantic-silos').on('click', function () {
        ...
        $.post(silAjax.ajaxurl, {
            ...
            $btn.prop('disabled', false).text(originalText);
            if (res.success) {
                var msg = '✅ Silos recalculés ! ' + (res.data.count || 0) + ' articles classés, ' + (res.data.bridges || 0) + ' ponts détectés.';
    ...
    $(document).on('click', '.sil-preview-btn', function () {
        ...
        $.post(silAjax.ajaxurl, {
            ...
            if (r.success && r.links && r.links.length > 0) {
                var html = '<div class="sil-preview-box"><div class="sil-preview-title">🔗 ' + r.links.length + ' lien(s) suggéré(s)</div>';
                r.links.forEach(function (link, idx) {
                    var id = 'link-' + postId + '-' + idx;

                    // Gestion affichage Cornerstone
                    var cornerstoneBadge = link.is_cornerstone ? ' <span style="background:#fef3c7; color:#d97706; padding:2px 6px; border-radius:4px; font-size:10px; border:1px solid #fcd34d; font-weight:bold;">⭐ PILIER</span>' : '';
    ...
    function updateDetailedPanel(node) {
        var html = '<div id="sil-lhf-warning-box" style="display:none; margin-top:15px; padding:10px; background-color:#f0fdf4; border:1px solid #86efac; border-radius:6px;">' +
            ...
            '<a href="' + (typeof silAjax !== 'undefined' ? silAjax.admin_url : '') + '?page=smart-internal-links&target_post=' + node.id + '" id="sil-boost-lhf-btn" class="sil-btn sil-btn-primary sil-btn-sm" style="width:100%; justify-content:center; background:#16a34a; border-color:#15803d; text-decoration:none;">🔗 Booster avec des liens internes</a>' +
            '</div>' +
    ...
    // Action Pont Sémantique (Workflow Hybride)
    $(document).on('click', '.sil-opp-bridge', function() {
        ...
        $.post(silAjax.ajaxurl, {
            ...
            $btn.prop('disabled', false).text('🌉 Créer un pont');
            if (response.success) {
                // Ici, on appelle la fonction d'affichage de modale déjà définie dans admin.js ou graph.js
                if (typeof openBridgeModal === 'function') {
                    openBridgeModal(response.data);
                } else {
                    alert("Prompt généré ! Copiez-le depuis la console ou utilisez la cartographie pour finaliser.");
    ...
    $(document).on('click', '#sil-run-semantic-audit', function(e) {
        ...
        $.post(silAjax.ajaxurl, { action: 'sil_run_semantic_audit', nonce: silAjax.nonce }, function(r) {
            $btn.prop('disabled', false);
        ...
        intruders.forEach(item => {
            ...
            html += `
                <tr>
                    ...
                    <td>
                        <a href="post.php?post=${item.id}&action=edit" class="button button-small" target="_blank">✏️ Corriger</a>
    ...
    function runEmbeddingBatch() {
        ...
        $.post(silAjax.ajaxurl, { action: 'sil_index_embeddings_batch', nonce: silAjax.nonce }, function(r) {
            if (r && r.success) {
                if (r.data.finished) {
    ...
    function updateIndexingStats() {
        $.post(silAjax.ajaxurl, { action: 'sil_get_indexing_status', nonce: silAjax.nonce }, function(r) {
            if (r && r.success) {
                const p = (r.data.indexed / r.data.total) * 100;
                $('#sil-indexing-bar').css('width', p + '%');
                $('#sil-indexing-stats').text(r.data.indexed + ' / ' + r.data.total + ' articles indexés');
            }
        });
    }
