jQuery(document).ready(function ($) {
    ...
    function renderCytoscape(data, maxPagerank, maxWeight, siloLabels) {
        ...
        cy.on('tap', 'edge', function(evt) {
            ...
            $sidebar.show();
            $content.html('<div style="padding:40px; text-align:center;"><div class="spinner is-active" style="float:none; margin:0 auto 20px;"></div><p style="color:#64748b; font-size:13px;">Récupération du contexte du lien...</p></div>');
            ...
            $.post(silGraphData.ajaxurl || ajaxurl, {
                ...
                if (!response.success) {
                    $content.html('<p style="color:#d63638; padding:20px;">Erreur : ' + escapeHtml(response.data || 'Impossible de charger le contexte.') + '</p>');
                    return;
                }
                const d = response.data;

                let html = '<div style="border-top:4px solid #3b82f6; padding-top:12px;">';
                html += '<h4 style="margin:0 0 10px; font-size:14px; color:#1d2327;">🔍 Inspection du Lien</h4>';
                
                // Connection Info
                html += '<div style="display:flex; align-items:center; gap:8px; margin-bottom:15px; background:#f8fafc; padding:10px; border-radius:8px; border:1px solid #e2e8f0;">';
                html += '<div style="flex:1; text-align:right;"><small style="display:block; color:#64748b; font-size:9px;">DE</small><strong style="font-size:11px;">' + escapeHtml(d.source_title) + '</strong></div>';
                html += '<div style="color:#94a3b8;">➔</div>';
                html += '<div style="flex:1;"><small style="display:block; color:#64748b; font-size:9px;">VERS</small><strong style="font-size:11px;">' + escapeHtml(d.target_title) + '</strong></div>';
                html += '</div>';
 
                // Semantic Leak Alert
                if (d.is_leak) {
                    const isSafe = d.leak_percent < d.leak_threshold;
                    const bgColor = isSafe ? '#eff6ff' : '#fef2f2';
                    const borderColor = isSafe ? '#bfdbfe' : '#fca5a5';
                    const titleColor = isSafe ? '#1d4ed8' : '#b91c1c';
                    const detailColor = isSafe ? '#1e40af' : '#7f1d1d';
                    const icon = isSafe ? 'ℹ️' : '⚠️';
                    const statusText = isSafe ? 'Fuite Tolérée' : 'Fuite Sémantique !';

                    html += '<div style="background:' + bgColor + '; border:1px solid ' + borderColor + '; border-radius:6px; padding:10px; margin-bottom:15px; font-size:12px; display:flex; align-items:flex-start; gap:8px;">';
                    html += '<span style="font-size:18px;">' + icon + '</span>';
                    html += '<div><strong style="color:' + titleColor + ';">' + statusText + ' (' + d.leak_percent + '%)</strong><br>';
                    html += '<span style="color:' + detailColor + '; font-size:11px;">Ce lien traverse deux silos : <strong>' + escapeHtml(d.source_silo_label) + '</strong> ➔ <strong>' + escapeHtml(d.target_silo_label) + '</strong>.</span><br>';
                    html += '<small style="color:' + detailColor + '; opacity:0.8; font-size:10px;">Seuil de tolérance : ' + d.leak_threshold + '%</small></div>';
                    html += '</div>';
                } else {
                    html += '<div style="background:#f0fdf4; border:1px solid #86efac; border-radius:6px; padding:10px; margin-bottom:15px; font-size:12px; display:flex; align-items:flex-start; gap:8px;">';
                    html += '<span style="font-size:18px;">✅</span>';
                    html += '<div><strong style="color:#166534;">Maillage Hermétique</strong><br><span style="color:#14532d; font-size:11px;">Ce lien reste à l\'intérieur du silo : <strong>' + escapeHtml(d.source_silo_label) + '</strong>.</span></div>';
                    html += '</div>';
                }
  
                // Semantic Proximity
                html += '<div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:8px; padding:10px; margin-bottom:15px; display:flex; justify-content:space-between; align-items:center;">';
                html += '<span style="font-size:11px; color:#64748b;">Proximité Sémantique (Carte)</span>';
                html += '<div style="display:flex; align-items:center; gap:5px;">';
                html += '<div style="width:60px; height:6px; background:#e2e8f0; border-radius:3px; overflow:hidden;"><div style="width:' + d.proximity + '%; height:100%; background:#3b82f6;"></div></div>';
                html += '<strong style="font-size:12px; color:#1e293b;">' + d.proximity + '%</strong>';
                html += '</div></div>';

                // Context & Anchor
                html += '<div style="background:#fff; border:1px solid #e2e8f0; border-radius:8px; padding:12px; margin-bottom:15px; box-shadow:0 1px 2px rgba(0,0,0,0.05);">';
                html += '<strong style="font-size:10px; text-transform:uppercase; color:#64748b; display:block; margin-bottom:8px;">Ancre et Contexte</strong>';
                html += '<div style="font-size:13px; color:#1e293b; line-height:1.6; font-style:italic;">';
                // Context parts are already stripped of HTML in PHP, but we still secure the anchor
                html += '"...' + escapeHtml(d.context_prev || '') + ' <span style="background:#fde047; padding:2px 4px; border-radius:3px; font-weight:700; font-style:normal;">' + escapeHtml(d.anchor) + '</span> ' + escapeHtml(d.context_next || '') + '..."';
                html += '</div>';
                html += '</div>';
                ...
                html += '<button class="button ' + nfClass + ' sil-toggle-nofollow-btn" data-source="' + sourceId + '" data-target-url="' + escapeHtml(d.target_url) + '" style="font-size:12px;">' + nfText + '</button>';
                
                // Delete Link
                html += '<button class="button sil-delete-link-edge-btn" data-source="' + sourceId + '" data-target-url="' + escapeHtml(d.target_url) + '" style="font-size:12px; color:#dc2626; border-color:#fca5a5;">🗑️ Supprimer le lien</button>';
                
                html += '</div>';
 
                // Navigation
                html += '<div style="margin-top:20px; border-top:1px solid #e2e8f0; padding-top:15px; display:flex; gap:8px;">';
                if (d.source_edit_url) html += '<a href="' + escapeHtml(d.source_edit_url) + '" target="_blank" class="button button-secondary" style="flex:1; text-align:center; text-decoration:none; font-size:11px;">Editer Source</a>';
                if (d.target_edit_url) html += '<a href="' + escapeHtml(d.target_edit_url) + '" target="_blank" class="button button-secondary" style="flex:1; text-align:center; text-decoration:none; font-size:11px;">Editer Cible</a>';
                html += '</div>';
