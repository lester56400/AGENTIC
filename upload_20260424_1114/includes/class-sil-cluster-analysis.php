...
class SIL_Cluster_Analysis {
    ...
    public function get_graph_data($force_refresh = false) {
        ...
        $cache_key = 'sil_graph_cache_v9_0';
        if (!$force_refresh) {
        ...
        // --- NOUVEAU: Map des ancres entrantes (pour enrichir le GEM) ---
        $incoming_anchors = [];
        foreach ($edges_data as $edge) {
            $target = (string)$edge['data']['target'];
            $anchor = $edge['data']['anchor'] ?? '';
            if ($anchor && mb_strlen($anchor) > 1) {
                if (!isset($incoming_anchors[$target])) $incoming_anchors[$target] = [];
                $clean = mb_strtolower(trim(wp_strip_all_tags($anchor)));
                if ($clean && !in_array($clean, $incoming_anchors[$target]) && count($incoming_anchors[$target]) < 5) {
                    $incoming_anchors[$target][] = $clean;
                }
            }
        }
        ...
        $all_meta = $wpdb->get_results("SELECT post_id, meta_key, meta_value FROM {$wpdb->postmeta} WHERE meta_key IN ('_sil_is_cornerstone', '_sil_embedding', '_sil_gsc_data', '_sil_gsc_index_status') AND post_id IN (" . implode(',', $post_ids_arr) . ")", ARRAY_A);
        ...
        foreach ($nodes_data as &$n) {
            ...
            if ((string)$best_cid !== (string)$cid) {
                ...
                $n['data']['ideal_cluster_label'] = $silo_titles[$best_cid] ?? "Silo $best_cid";
        ...
        foreach ($nodes_data as $n) {
            ...
            if (!empty($emb) && is_array($emb)) {
                $all_embeddings_for_pca[$pid] = $emb;
            }
        }
        ...
        set_transient('sil_graph_cache_v9_0', $final_data, 3 * HOUR_IN_SECONDS);
    ...
    public static function compute_2d_projection($embeddings) {
        ...
        for ($i = 0; $i < $n; $i++) {
            $coords[$ids[$i]] = [
                'x' => round(50 + (($raw_x[$i] - $min_x) / $range_x) * 1900, 2),
                'y' => round(50 + (($raw_y[$i] - $min_y) / $range_y) * 1900, 2),
            ];
        }
