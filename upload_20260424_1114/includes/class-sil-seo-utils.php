...
class SIL_SEO_Utils {
	...
	/**
	 * Extracts primary keywords for a post (GSC queries + Clean Title).
	 * 
	 * @param int $post_id
	 * @return array List of unique strings.
	 */
	public static function get_post_keywords( $post_id ): array {
		$gsc_raw = get_post_meta( $post_id, '_sil_gsc_data', true );
		$gsc = is_array( $gsc_raw ) ? $gsc_raw : json_decode( $gsc_raw, true );
		$keywords = [];

		if ( $gsc && isset( $gsc['top_queries'] ) && is_array( $gsc['top_queries'] ) ) {
			foreach ( array_slice( $gsc['top_queries'], 0, 5 ) as $q ) {
				$kw = isset( $q['query'] ) ? $q['query'] : ( isset( $q['keys'][0] ) ? $q['keys'][0] : '' );
				if ( $kw ) $keywords[] = $kw;
			}
		}

		// Clean Title as fallback/addition
		$keywords[] = wp_specialchars_decode( get_the_title( $post_id ), ENT_QUOTES );
